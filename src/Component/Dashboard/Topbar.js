...
export default function Topbar(){
    ...
    const[acount_notifications,setAcountNotfication]=useState(0)
    ...
    async function getNotification(){         
        try{
          ...
          .then(res=>{setNotfication(res.data);
            ...
            if(res.data){
                setAcountNotfication(0)
                res.data.map((item)=>
                    setAcountNotfication(perv => (item.is_read===0)?(perv+1):(perv) )
                )
